// Package tests holds the black-box tests for the iptolocation package.
package tests
...
func TestLookupNilService(t *testing.T) {
	var s *iptolocation.Service // never opened
	if _, err := s.Lookup("8.8.8.8"); !errors.Is(err, iptolocation.ErrUnavailable) {
...
// (this test file lives in <root>/iptolocation/tests/). "" if not found.
func resolveDB(p string) string {
	...
	root := filepath.Dir(filepath.Dir(filepath.Dir(thisFile))) // tests → iptolocation → root
...
//
//	set -a; . ./.env; set +a; go test ./iptolocation/tests -run Integration
