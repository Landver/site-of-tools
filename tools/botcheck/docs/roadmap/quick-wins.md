...
|---|---|---|---|
| G01 | Expand userAgentData high-entropy hints + platformVersion coherence | trivial | Shipped 2026-07-17, trimmed same day: `navigator.userAgentData.platform` is read directly and `fullVersionList` is requested via `getHighEntropyValues(['fullVersionList'])`; `ua_chrome_version_mismatch` compares the UA's Chrome/NNN major against the `Chromium` (falling back to `Google Chrome`) `fullVersionList` entry. `platformVersion`/`uaFullVersion`/`architecture`/`bitness`/`model` were requested briefly then dropped as unused (YAGNI) — no rule compares UA-embedded OS version vs `userAgentData.platformVersion`, since `platformVersion` isn't collected. |
| G02 | navigator.productSub / oscpu / buildID / pdfViewerEnabled | trivial | `productSub` shipped as an engine-consistency rule. `pdfViewerEnabled` was tried and dropped: it fires on ordinary desktop Chrome with the "Download PDFs" setting or the `AlwaysOpenPdfExternally` enterprise policy, a user preference not a headless tell, and correlates with `empty_plugins` — see [changelog.md](changelog.md). `oscpu`/`buildID` were skipped. |
