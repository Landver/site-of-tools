...
These items appear scattered across the category files; grouped here so they
...
- **Blocked by topology (edge/TLS):** TLS JA3/JA4 (G27), HTTP/2 frame fingerprint
  ...
  cross-layer OS-coherence rule that depends on them (G48). See
  [network-layer.md](network-layer.md) and [scoring-fusion.md](scoring-fusion.md).
  All blind as long as nginx/Cloudflare terminate TLS in front of Go.
- **Needs a stored corpus (Mongo-backed since wave 2 — the fingerprint corpus
  ...
  See [ip-reputation.md](ip-reputation.md), [reporting-ux.md](reporting-ux.md),
...
- **Off-brand non-goals for a self-test tool:** enforcement / inline WAF (G61),
  ...
  server-side port scanning (G32). See [enforcement.md](enforcement.md),
