...
- **Domain (`botcheck_test.go`)** — construct canned `Signals` (no HTTP, no BINs),
  ...
  `Checks` slice locks in *which* signals fired, not just count.
- **Handler (`handler_test.go`)** — `httptest`: `POST /check` a JSON fingerprint,
  ...
  for `Accept: text/html`), w/ fake `iptools.Looker` (no PX12 BIN in CI) whose
  zero value returns nil result, so IP signals absent & only client half of
  fingerprint scored.
- **Corpus (`corpus_test.go`)** — `FingerprintHash` determinism (server-observed
  ...
  good-bot suppression, & nil-safe disabled store. Live Mongo round-trip +
...
`rules`/`ruleExplanations`: every consistency-tier rule has subgroup, every
rule ID — all 68 currently implemented, plus 1 remaining reserved ID,
`system_color_headless` — has `ruleExplanations` entry (G55 coverage guard).
Third guard lives black-box, `TestEveryRuleCanFire` in
`tests/botcheck_test.go` (added 2026-07-21): every rule `Evaluate` emits must
have fire-path fixture in `ruleFirePaths` that actually trips it (& stays
silent on clean fixture), so dead predicate — like ones the
...
[testing/checks/](testing/checks/README.md) — each of 68 per-check files
lists its own scorer-test coverage instead of this page enumerating
all 68 (a few, e.g. `framework_globals` & `ch_platform_mismatch`, currently
have none beyond blanket explanation-presence & fire-path guards above).
...
tests *scorer*, not *collector*. Exactly what let real bug in
`webglGPU()` ship & survive every Go test run — see
[testing/findings/2026-07-19-webglgpu-bug-fixed.md](testing/findings/2026-07-19-webglgpu-bug-fixed.md)
for incident & why npm-based [testing/](testing/README.md) harness exists to
cover that gap.
