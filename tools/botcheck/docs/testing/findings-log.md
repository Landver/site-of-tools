...
| [`tostring_proxy` FIXED: V8's stack-frame format outran both stealth's stripper and our detector](findings/2026-07-19-tostring-proxy-alias-frame-fix.md) | The nested-double-throw idea from next-steps item 3 wasn't even needed — a single illegal call already leaked stealth's raw `newHandler` proxy-trap frame, because current V8 formats it as a bracket alias (`[as apply]`) that neither `stripProxyFromErrors`' anchor search nor our own regex matched. Broadened the detector's pattern; verified live: stealth's score dropped 25→0, the other three frameworks (Playwright, real-Chrome-binary Selenium, raw CDP) unchanged. |
| [`chrome_runtime_tamper` (item 2): deprioritized, not closed](findings/2026-07-19-chrome-runtime-tamper-deprioritized.md) | Neither browser tool available this session can supply the genuine-consumer-Chrome sample this item has been waiting on — the in-app Browser pane turned out to be an Electron embed, not Chrome. Recommend deprioritizing regardless: stealth's chrome.runtime evasion was already shown to only matter against naive (non-stealth) bots, so even a clean organic sample wouldn't make the tightened check worth shipping. |
