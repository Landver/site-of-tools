...
Botcheck reads two Mongo collections in shared `site-of-tools` database: its
...
Rolling corpus of fingerprint sightings in MongoDB (`botcheck_fingerprints`
...
`(hash, IP, ts)` collection feeds **two** rules, spatial & temporal reads
of same data:

- **Reuse (G41/G42, `fingerprint_reuse`, consistency −25):** counts how many
  **distinct IPs** presented that exact hash. Five+ trips the rule — the
  scraping-farm catch (farm locks one browser fingerprint, rotates its proxy
  pool, incolumitas ScrapingBee case); one person roaming networks
  never reaches five IPs in a month. Verified crawler fleets share one
  fingerprint by design, so rule suppressed for them (G36 — see
  [roadmap/ip-reputation.md](roadmap/ip-reputation.md)).
- **Churn (G43, `ip_fingerprint_churn`, soft 8):** inverse read — counts how
  many **distinct fingerprints** one IP presented within short rolling window
  (`churnWindow`, 10 min), via `Corpus.DistinctHashesByIP`. Eight+ = a
  fingerprint-randomising client (or busy shared egress). Soft, so a corporate
  NAT legitimately showing many browsers never docks a lone visitor — only
  bites in a ≥3 soft cluster.

Corpus deliberately minimal & self-pruning: `ts` TTL index expires every
sighting after 30 days, hash is one-way (raw fingerprint never stored), and —
like iptools history — disabled Mongo (empty `MONGODB_URI`) turns whole
thing into no-op: `FingerprintIPs`/`FingerprintChurn` stay 0, both rules stay
silent, score unchanged. Domain scorer stays pure: both counts arrive as
...
service/script/workflow can write flagged IPs into it, any consumer can
...
**Schema** (corpus's public contract — external writers match these field
names, snake_case bson):

| field | meaning |
|---|---|
| `ip` | flagged address for single-IP entry, or CIDR string itself for a netblock entry (Spamhaus DROP) — CIDR is that record's identity |
| `source` | what flagged it — `"ipsum"`, `"spamhaus-drop"`, `"rate-limiter"`, `"manual"`, … |
| `reason` | free-text why (optional) |
| `count` | optional confidence/occurrence count (ipsum: how many of its 30+ feeds list the IP); 0 = source carries none (true for Spamhaus DROP, binary presence not count) |
| `range_start` / `range_end` | inclusive IPv4 bounds (see package-internal `ipv4RangeBounds` helper in [`cidr.go`](../../iptools/cidr.go)) for a netblock entry — one document covers whole CIDR instead of one row per address. Omitted for plain single-IP entry |
| `meta` | source-specific extras, so nothing a richer feed carried is lost (ipsum stashes feed timestamp + URL; Spamhaus DROP stashes copyright notice + timestamp + terms URL + per-record sblid/rir) |
| `created_at` | set once, on first insert (immutable — via `$setOnInsert`) |
| `updated_at` | refreshed on every touch; drives TTL |

**Indexes:** a **TTL** on `updated_at` (60 days — owner's spec: entry not
refreshed within two months self-prunes, so reputation decays once IP falls
off every feed), a **unique `(ip, source)`** compound, and a **sparse
`(range_start, range_end)`** compound (only netblock entries carry those
fields, so sparse keeps ~100k-plus single-IP rows out of it). Unique
compound = the "don't lose data" guarantee — each source keeps its *own*
record per IP/CIDR, so daily feed syncs never clobber a manual/other-service
...
(`BlockList.ShouldSync`, since both feeds want identical 24-hour cadence —
threshold is interval *minus* an hour of slack, not bare interval,
...
- **ipsum** ([`iptools/ipsum.go`](../../iptools/ipsum.go), `RunIPsumSync`
  ...
  bulk-upserts every IP under source `ipsum`, preserving occurrence count.
- **Spamhaus DROP** ([`iptools/spamhaus.go`](../../iptools/spamhaus.go),
  ...
  keeping copyright notice + date "with the file and data." DROP lists
  ...
  per address isn't an option. Each CIDR record stored as one document with
  `range_start`/`range_end` bounds instead, its own `meta` carries the
  ...
  isn't worth the complexity right now. Site footer also credits Spamhaus
...
Both syncs advance touched entries' `updated_at` on every refresh, keeping
still-listed IPs/netblocks alive against TTL; an entry that drops off its
...
looked-up IP (any address, not just egress), renders it in the "proxy /
blocklist / network" result card + JSON `blocklist` field. Either way,
...
with an `$or` — callers never need to know which kind matched.
...
nil-safety as fingerprint corpus: disabled Mongo → nil `BlockList` → `Check`
returns empty → rule silent → pure `Evaluate` still needs no DB. See
[checks/ip_blocklisted.md](testing/checks/ip_blocklisted.md) for fire logic
