...
v4) skip older payloads, so returning visitor w/ stale cached collector never
reads as tampered.
...
- **Hard automation tells** — `navigator.webdriver`; automation-framework
  ...
  before assuming it still works as strong tell.
- **Lie / tamper detection** — shallow `Function.prototype.toString()`
  ...
  `enumerable: true`, ECMA-262 built-ins not), call/new `TypeError`
  ...
  `sendMessage`/`connect` = native non-constructors — no own `prototype`, and
  ...
  window keys = bolted on after page setup). **2026-07-19 finding:** all
...
- **Classic headless tells** — impossible permission state (`prompt` while
  ...
  `screen == avail`, common & never flagged) /
...
- **The cross-check most free tools skip (our differentiator)** —
  ...
  specifically). CreepJS/Electron catch: UA-string spoof leaving
...
- **Engine constants** — `navigator.productSub` fixed per-engine value
  (`20030107` on WebKit/Blink, `20100101` on Gecko); value disagreeing w/
  engine UA claims (derived via same `engineFromUA` helper, so iOS browsers
  correctly treated as WebKit) = patched-runtime tell. Second, independent
...
- **WebRTC candidate IPs** — `RTCPeerConnection` against public STUN server
  ...
  host candidate ≠ egress = normal NAT — only egress's own address family
...
- **The v4 `env` section (G15/G21)** — one additive object of cheap
  environment/API probes, all fail-to-absent (missing key = "not supplied,"
  ...
  (`matchmedia_missing`, `netinfo_incoherent`); rest = entropy for raw dump,
