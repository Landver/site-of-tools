...
- Our **shipped** implementation, read as ground truth (not the design doc):
  [`botcheck/scoring.go`](../scoring.go) (the 66 detection rules),
...
| G14 | CDP / navigator recheck in a Service Worker context | bot.incolumitas, CreepJS | low · low · **Shipped** | Run CDP and navigator consistency checks inside a Service Worker (not just a Web Worker); bot.incolumitas's inconsistentServiceWorkerNavigatorPropery flagged an Electron/CDP browser whose main-thread webdriver read clean. → **Shipped 2026-07-18:** `/botcheck-sw.js` now also reports `navigator.webdriver` and runs the same CDP Error.stack trap the worker probe uses (still no fetch handler, still unregistered after the probe). New rules: `webdriver_sw` (hard) and `cdp_sw_only` (consistency, guarded so it never double-counts with `cdp_both`/`cdp_main_only`). |
...
- **Needs a stored corpus (Mongo-backed since wave 2 — the fingerprint corpus
  shipped G41/G42):** crowd rarity & entropy (G40, G58), fuzzy/LSH hashing (the
  deferred half of G42), request velocity (G43), persistent visitor ID (G47).
  (Returning-visitor history, G46, shipped localStorage-only instead — no corpus
  needed.)
