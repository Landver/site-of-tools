...
// historyTTL: how long recorded lookup stays. Long enough for useful
...
// HistoryEntry is one recorded lookup: queried IP, salient facts, when.
// Deliberately projection of Result, not whole struct → enough to show +
// replay (click IP to re-run), no more.
...
// History is persistence layer for IP-lookup history: repository below
...
// NewHistory builds repository from app database handle. Nil db (Mongo
// disabled) → returns nil, nil-safe disabled store. TTL index best-effort:
// failure only forfeits auto-expiry → non-fatal.
...
// (background goroutine, own bounded context) → recording never adds latency
// to, or fails, lookup visitor waiting on: lost record OK, slowed lookup not.
// Volume low (user-initiated lookups only) → plain goroutine enough;
// higher-volume request log uses bounded queue instead.
...
// special-casing Mongo off. created_at TTL index also serves this descending
// sort (Mongo scans it in reverse) → no second index.
